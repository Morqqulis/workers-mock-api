...
function MousePosition(onMove?: (x: number, y: number) => void): MousePosition {
...
export const Particles: React.FC<ParticlesProps> = ({
	...
	const onMouseMove = useCallback((x?: number, y?: number) => {
		if (canvasRef.current) {
			...
			const mx = (x ?? mousePosition.x) - rect.left - w / 2
			const my = (y ?? mousePosition.y) - rect.top - h / 2
	...
	// Передаём onMouseMove в MousePosition, чтобы обновлять mouse.current при движении мыши
	...
	const drawParticles = () => {
		circles.current.forEach((circle, i) => {
			...
			const closestEdge = edge.reduce((min, curr) => (curr < min ? curr : min))
			...
			if (remapClosestEdge < 1) {
				...
				circle.alpha -= circle.targetAlpha / staticity
				if (circle.alpha < 0) {
					circle.alpha = 0
				}
			}
